...
pub const UPD_PDN_CONN_SET_REQ: u8 = 200;

// Definition of GTPv2-C Update PDN Connection Set Request Message

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct UpdatePdnConnectionSetRequest {
...
impl Default for UpdatePdnConnectionSetRequest {
    fn default() -> Self {
        let hdr = Gtpv2Header {
            msgtype: UPD_PDN_CONN_SET_REQ,
            teid: Some(0),
            ..Default::default()
        };
...
impl Messages for UpdatePdnConnectionSetRequest {
    ...
    fn unmarshal(buffer: &[u8]) -> Result<Self, GTPV2Error> {
        let mut message = UpdatePdnConnectionSetRequest::default();
        match Gtpv2Header::unmarshal(buffer) {
            Ok(i) => message.header = i,
            Err(j) => return Err(j),
        }

        if message.header.msgtype != UPD_PDN_CONN_SET_REQ {
...
fn test_update_pdn_connection_set_req_unmarshal() {
    ...
    let decoded = UpdatePdnConnectionSetRequest {
        ...
        ..UpdatePdnConnectionSetRequest::default()
    };
    let message = UpdatePdnConnectionSetRequest::unmarshal(&encoded).unwrap();
