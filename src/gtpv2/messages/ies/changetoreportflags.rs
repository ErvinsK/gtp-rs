// Change to Report Flags IE - according to 3GPP TS 29.274 V15.9.0 (2019-09)
...
impl Default for ChangeToReportFlags {
    fn default() -> ChangeToReportFlags {
        ChangeToReportFlags { t: CHNG_TO_RPRT_FLAGS, length:CHNG_TO_RPRT_FLAGS_LENGTH as u16, ins:0, tzcr:false, sncr:false }        
    }
}
...
impl IEs for ChangeToReportFlags {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV2Error> {
        if buffer.len()>= CHNG_TO_RPRT_FLAGS_LENGTH + MIN_IE_SIZE {
            let mut data = ChangeToReportFlags::default();
...
fn change_to_report_flags_ie_unmarshal_test () {
    ...
    let decoded = ChangeToReportFlags { t:CHNG_TO_RPRT_FLAGS, length: CHNG_TO_RPRT_FLAGS_LENGTH as u16, ins:0, tzcr: true, sncr:false };
