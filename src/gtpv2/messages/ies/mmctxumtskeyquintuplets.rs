...
// MM Context UMTS Key and Quintuplets IE Type

pub const MMCTXUMTSKQ: u8 = 106;

// MM Context UMTS Key and Quintuplets IE implementation

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MmContextUmtsKeyQuintuplets {
    pub t: u8,
    pub length: u16,
    pub ins: u8,
...
impl Default for MmContextUmtsKeyQuintuplets {
    fn default() -> Self {
        MmContextUmtsKeyQuintuplets {
            ...
            length: 0,
            ins: 0,
            sec_mode: SecurityMode::UmtsKeyAndQuintuplets,
            ksi: 0,
...
impl From<MmContextUmtsKeyQuintuplets> for InformationElement {
    fn from(i: MmContextUmtsKeyQuintuplets) -> Self {
        InformationElement::MmContextUmtsKeyQuintuplets(i)
    }
}
        ...
        if buffer.len() >= 37 + MIN_IE_SIZE {
                ...
                i if i<=5 => {
                    ...
                    for j in 0..authi {
                        if let Ok(ie) = AuthQuintuplet::unmarshal(&buffer[cursor..]) {
                            ...
                            return Err(GTPV2Error::IEIncorrect(MMCTXUMTSKQ));
                        }
                    }
            ...
            } 
            {
                ...
                match len {
                    ...
                    0x00 => (),
                    _ => return Err(GTPV2Error::IEInvalidLength(MMCTXUMTSKQ)),
                }
            }
            Ok(data)
        } else {
            Err(GTPV2Error::IEInvalidLength(MMCTXUMTSKQ))
        }
    }
