// S1-U Data Forwarding IE - according to 3GPP TS 29.274 V15.9.0 (2019-09)
            ...
            if check_tliv_ie_buffer(data.length, buffer) {
                ...
                match buffer[5] {
                    0x04 => {
                        ...
                        data.sgw_s1u_teid = u32::from_be_bytes([buffer[10], buffer[11], buffer[12], buffer[13]]);
                    },
                    0x10 => {
                        if buffer.len() >= 0x16 {
                            let mut dst = [0; 16];
                            ...
                            data.sgw_s1u_teid = u32::from_be_bytes([buffer[22], buffer[23], buffer[24], buffer[25]]);
                        } else {
...
fn s1udf_ie_ipv4_unmarshal_test() {
    let encoded_ie: [u8; 14] = [0x5b, 0x00, 0x0a, 0x00, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
...
fn s1udf_ie_wrong_ip_address_type() {
    let encoded_ie: [u8; 14] = [0x5b, 0x00, 0x0a, 0x00, 0x01, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
    let i = S1udf::unmarshal(&encoded_ie);
