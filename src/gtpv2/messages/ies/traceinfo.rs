...
impl IEs for TraceInformation {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV2Error> {
        if buffer.len()>=MIN_IE_SIZE+TRACEINFO_LENGTH {
            let mut data=TraceInformation::default();
...
fn trace_info_ie_marshal_test() {
    let decoded = TraceInformation { t:TRACEINFO, length: TRACEINFO_LENGTH as u16, ins:0,  
                                        mcc:999, mnc:1, 
                                        trace_id: 0xfffffa, 
    ...
    let encoded:[u8;38] = [0x60, 0x00, 0x22, 0x00, 0x99, 0xf9, 0x10, 0xff, 0xff, 0xfa, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa, 0xaa, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8b, 0x07, 0x85, 0xb8];
