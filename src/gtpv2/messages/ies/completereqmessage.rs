...
impl Default for CompleteRequestMessage {
    fn default() -> Self {
        CompleteRequestMessage { t: COMPLETE_REQ_MSG, length:0, ins:0, message:RequestMessage::Spare}
    }
}
...
fn complete_request_msg_ie_marshal_test () {
    ...
    let decoded = CompleteRequestMessage { t:COMPLETE_REQ_MSG, length: 6, ins: 0, message: RequestMessage::AttachRequest(vec!(0xaa, 0xbb, 0xcc, 0xdd, 0xee)) };
...
fn complete_request_msg_ie_unmarshal_test () {
    let encoded:[u8;10]=[0x74, 0x00, 0x06, 0x00, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee];
    let decoded = CompleteRequestMessage { t:COMPLETE_REQ_MSG, length: 6, ins: 0, message: RequestMessage::AttachRequest(vec!(0xaa, 0xbb, 0xcc, 0xdd, 0xee)) };
    assert_eq!(CompleteRequestMessage::unmarshal(&encoded).unwrap(), decoded);
}