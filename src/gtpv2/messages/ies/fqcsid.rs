// FQ-CSID IE - according to 3GPP TS 29.247 V15.9.0 (2019-09)
            ...
            match buffer[4]>>4 {
                0 => {
                    ...
                    if check_tliv_ie_buffer((cursor-4) as u16, buffer) {
                        data.nodeid = NodeId::V4(Ipv4Addr::from([buffer[5], buffer[6], buffer[7], buffer[8]]));
                        data.csid = buffer[9..cursor].to_vec().chunks(2).map(|x| u16::from_be_bytes([x[0],x[1]])).collect();
                    } else {
                        return Err(GTPV2Error::IEInvalidLength(FQCSID));
                    }
                },
                1 => {
                    ...
                    if check_tliv_ie_buffer((cursor-4) as u16, buffer) {
                        ...
                        data.csid = buffer[21..cursor].to_vec().chunks(2).map(|x| u16::from_be_bytes([x[0],x[1]])).collect();
                    } else {
                        return Err(GTPV2Error::IEInvalidLength(FQCSID));
                    }
                },
                2 => {
                    let cursor = (9+2*(buffer[4]&0x0f)) as usize;
                    if check_tliv_ie_buffer((cursor-4) as u16, buffer) {
                        data.nodeid = NodeId::NwAddr(u32::from_be_bytes([buffer[5], buffer[6], buffer[7], buffer[8]]));
                        data.csid = buffer[9..cursor].to_vec().chunks(2).map(|x| u16::from_be_bytes([x[0],x[1]])).collect();
                    } else {
                        return Err(GTPV2Error::IEInvalidLength(FQCSID));
                    }
...
fn fqcsid_ie_ipv4_unmarshal_test () {
    let encoded:[u8;11]=[0x84, 0x00, 0x07, 0x00, 0x01, 0x8b, 0x07, 0x85, 0xb8, 0xff, 0xff];
    let decoded = Fqcsid {  t:FQCSID,
                                    length:7, 
                                    ins:0, 
...
fn fqcsid_ie_ipv6_unmarshal_test () {
    let encoded:[u8;25]=[0x84, 0x00, 0x15, 0x00, 0x12, 
...
fn fqcsid_ie_nwaddr_unmarshal_test () {
    let encoded:[u8;11]=[0x84, 0x00, 0x07, 0x00, 0x21, 0x8b, 0x07, 0x85, 0xb8, 0xff, 0xff];
    let decoded = Fqcsid {  t:FQCSID,
                                    length:7, 
                                    ins:0, 
