// Transaction Identifier (TI) IE - according to 3GPP TS 29.274 V15.9.0 (2019-09)
...
impl Default for TransactionIdentifier {
    fn default() -> Self {
        TransactionIdentifier { t: TRANSACT_ID, length:1, ins:0, tid: TransactionId { flag:false, id:0} }
    }
}
...
impl IEs for TransactionIdentifier {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV2Error> {
        if buffer.len()>=MIN_IE_SIZE+1 {
            ...
            data.tid.flag = match buffer[4]>>7 {
                0 => false,
                _ => true,
            };
            if data.length > 1 {
...
fn transaction_id_ie_marshal_test() {
    let decoded=TransactionIdentifier { t: TRANSACT_ID, length:2, ins:0, tid: TransactionId { flag:true, id: 7 }};
