// Delay Value IE - according to 3GPP TS 29.274 V15.9.0 (2019-09) 
...
impl Default for DelayValue {
    fn default() -> Self {
        DelayValue { t: DELAY_VALUE, length:DELAY_VALUE_LENGTH as u16, ins:0, value:0}
    }
}
...
impl IEs for DelayValue {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV2Error> {
        if buffer.len()>=MIN_IE_SIZE+DELAY_VALUE_LENGTH {
            let mut data=DelayValue::default();
...
fn delay_value_ie_marshal_test () {
    ...
    let decoded = DelayValue { t:DELAY_VALUE, length: DELAY_VALUE_LENGTH as u16, ins:0, value: 0xff };
