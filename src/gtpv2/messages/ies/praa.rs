// Presence Reporting Area Action IE - according to 3GPP TS 29.274 V15.9.0 (2019-09) 
...
impl Default for PresenceReportingAreaAction {
    fn default() -> Self {
        PresenceReportingAreaAction { t: PRAA , length:PRAA_LENGTH as u16, ins:0, inapra:false, action:0, prai:0, tai:vec!(), rai:vec!(), macro_enb:vec!(), home_enb:vec!(), ecgi:vec!(), sai:vec!(), cgi:vec!(), ext_macro_enb:vec!()}
    }
}
...
impl IEs for PresenceReportingAreaAction {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV2Error> {
        if buffer.len()>=MIN_IE_SIZE+PRAA_LENGTH {
            ...
            let (nbr_tai, nbr_rai, nbr_macro_enb, nbr_home_enb, nbr_ecgi, nbr_sai, nbr_cgi) = (buffer[8]>>4, buffer[8] & 0x0f, buffer[9] & 0x3f, buffer[10] & 0x3f, buffer[11] & 0x3f, buffer[12] & 0x3f, buffer[13] & 0x3f);
...
fn praa_ie_marshal_test () {
    let encoded:[u8;66]=[0xb1, 0x00, 0x3e, 0x00, 0x01, 0xff, 0xff, 0xff, 0x11, 0x01, 0x01, 0x01, 0x01, 0x01, 
    
    0x62, 0xf2, 0x10, 0x0b, 0xd9,           // Tai
...
let decoded = PresenceReportingAreaAction { t:PRAA, length: 62, ins:0, 
    inapra:false,
    action:1,
    prai: 0xffffff, 
    tai: vec!(Tai { mcc: 262, mnc:1, tac:0x0bd9}),
    rai: vec!(Rai { mcc: 262, mnc: 1, lac:0xffff, rac:0xaa}),
    macro_enb: vec!(MacroEnbId { mcc: 262, mnc: 1, macro_id:0x0fffff}),
    home_enb: vec!(MacroEnbId { mcc: 262, mnc: 1, macro_id:0x0fffff}),
...
fn praa_ie_unmarshal_test () {
    ...
    assert_eq!(PresenceReportingAreaAction::unmarshal(&encoded).unwrap(), decoded);
}
