...
impl IEs for Apco {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV2Error> {
        if buffer.len()>=MIN_IE_SIZE {
            let mut data=Apco::default();
            data.length = u16::from_be_bytes([buffer[1], buffer[2]]);
            data.ins = buffer[3];
            if  check_tliv_ie_buffer(data.length, buffer) {
                data.apco.extend_from_slice(&buffer[4..(data.length+4) as usize]);
                Ok(data)
            } else {
...
fn apco_ie_marshal_test () {
    let encoded:[u8;24]=[0xa3, 0x00, 0x14, 0x00, 0x80, 0x80, 0x21, 0x10, 0x01, 0x01, 0x00, 0x10, 0x81, 0x06, 0x00, 0x00, 0x00, 0x00, 0x83, 0x06, 0x00, 0x00, 0x00, 0x00];
    let decoded = Apco { t:APCO, length: 20, ins: 0, apco: vec!(0x80, 0x80, 0x21, 0x10, 0x01, 0x01, 0x00, 0x10, 0x81, 0x06, 0x00, 0x00, 0x00, 0x00, 0x83, 0x06, 0x00, 0x00, 0x00, 0x00) };
