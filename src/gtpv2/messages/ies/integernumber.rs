// Integer Number IE - according to 3GPP TS 29.274 V15.9.0 (2019-09) 
...
impl Default for IntegerNumber {
    fn default() -> Self {
        IntegerNumber { t: INT_NMBR, length:0, ins:0, number:vec!()}
    }
}
...
impl IEs for IntegerNumber {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV2Error> {
        if buffer.len()>=MIN_IE_SIZE {
            let mut data=IntegerNumber::default();
            data.length = u16::from_be_bytes([buffer[1], buffer[2]]);
            data.ins = buffer[3];
            if check_tliv_ie_buffer(data.length, buffer) {
                data.number.extend_from_slice(&buffer[4..]);
                Ok(data)
            } else {
...
fn integer_number_ie_marshal_test () {
    ...
    let decoded = IntegerNumber { t:INT_NMBR, length: 2, ins:0, number: vec!(0xff,0xff) };
