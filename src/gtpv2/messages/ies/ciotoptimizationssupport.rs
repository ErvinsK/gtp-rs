...
fn ciot_support_ie_marshal_test () {
    ...
    let decoded = CIoTOptimizationSupportIndication { t: CIOT_SUPPORT, length: CIOT_SUPPORT_LENGTH as u16, ins:0, sgnipdn: true, scnipdn: false, awopdn:false, ihcsi:true};
...
fn ciot_support_ie_unmarshal_test () {
    let encoded:[u8;5]=[0xc2, 0x00, 0x01, 0x00, 0x09];
    let decoded = CIoTOptimizationSupportIndication { t: CIOT_SUPPORT, length: CIOT_SUPPORT_LENGTH as u16, ins:0, sgnipdn: true, scnipdn: false, awopdn:false, ihcsi:true};
    assert_eq!(CIoTOptimizationSupportIndication::unmarshal(&encoded).unwrap(), decoded);
}