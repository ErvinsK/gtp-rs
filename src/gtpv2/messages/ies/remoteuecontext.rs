...
impl Default for RemoteUeContext {
    fn default() -> Self {
        RemoteUeContext { t: REMOTE_UE_CTX, 
                        length: 5,
                        ins:0,
...
fn remote_ue_ctx_ie_unmarshal_test () {
    ...
    let encoded:[u8;45]=[
                            ...
                            0xc1, 0x00, 0x05, 0x00, 0x01, 0x0a, 0xc2, 0xba, 0x34  
    ];
   let decoded = RemoteUeContext { 
    t: REMOTE_UE_CTX, 
    length: 41, 
    ins: 0, 
    user_id: RemoteUserId { t:REMOTE_USR_ID, length: 28, ins:0, 
        imsi:"901405101961873".to_string(),
        msisdn: Some("882285100125112".to_string()),
        imei: Some("8676480401323003".to_string()), },
    ue_ip: Some (
        RemoteUeIpInformation { t:REMOTE_UE_IP, length:5, ins:0, ip: RemoteIpAddress::V4(Ipv4Addr::new(10,194,186,52)) }
    ),
    };
