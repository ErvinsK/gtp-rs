...
fn test_error_indication_unmarshal () {
    ...
    let encoded:[u8;28] = [
        0x36,0x1a,0x0,0x14,0x0,0x0,0x0,0x0,
        0x49,0xca,0x0,0x40,0x1,0x10,0x0,0x0,
        0x10,0x0,0x0,0xff,0xff,0x85,0x0,0x4,
        0x64,0x75,0x82,0x35
    ];
    let decoded = ErrorIndication { 
        header: Gtpv1Header {
            ...
            npdu_number: None, 
            extension_headers:Some(vec!(ExtensionHeader::UDPPort(UDPPort { extension_header_type:UDP_PORT, length: UDP_PORT_LENGTH, udp_port: 4096 })))},
            teid_data: Teid { t: TEID_DATA, teid: 0xffff },
            peer_addr: GsnAddress { t: GSN_ADDRESS, length: 4, ip: IpAddr::V4(Ipv4Addr::new(100,117,130,53)) }, 
            private_extension: None };
...
fn test_error_indication_without_mandatory_ie_unmarshal () {
    let encoded:[u8;23] = [
        0x36,0x1a,0x0,0x0f,0x0,0x0,0x0,0x0,
        0x49,0xca,0x0,0x40,0x1,0x10,0x0,0x0,
        0x85,0x0,0x4,
        0x64,0x75,0x82,0x35
    ];
    assert_eq!(ErrorIndication::unmarshal(&encoded), Err(GTPV1Error::MessageMandatoryIEMissing));
}
