...
fn test_end_marker_unmarshal () {
    let encoded:[u8;12] = [0x32, 0xfe, 0x00, 0x04, 0x00, 0x00, 0x00, 0x02, 0x49, 0xca, 0x00, 0x00];
    let decoded = EndMarker { header: Gtpv1Header {msgtype:END_MARKER, length:4, teid:2, sequence_number:Some(18890), npdu_number: None, extension_headers:None}, private_ext: None };
...
fn test_end_marker_with_private_ext_unmarshal () {
    let encoded:[u8;20] = [0x32, 0xfe, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x49, 0xca, 0x00, 0x00, 0xff, 0x00, 0x05, 0x00, 0x08, 0x01, 0x02, 0x03];
    let decoded = EndMarker { header: Gtpv1Header {msgtype:END_MARKER, length:12, teid:0, sequence_number:Some(18890), npdu_number: None, extension_headers:None}, private_ext: Some(PrivateExtension { t:PRIVATE_EXTENSION, length:5, extension_id:8, extension_value: vec![1,2,3]})};
    assert_eq!(EndMarker::unmarshal(&encoded).unwrap(),decoded);
}
