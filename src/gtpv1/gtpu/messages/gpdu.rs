...
impl Messages for Gpdu {
    ...
    fn unmarshal (buffer: &[u8]) -> Result<Self, GTPV1Error> {
        ...
        if (message.header.length+8) as usize<=buffer.len() {
            message.tpdu.extend_from_slice(&buffer[message.header.get_header_size()..(message.header.length+8) as usize]); 
            Ok(message)
        } else {
...
fn test_gpdu_unmarshal () {
    let encoded:[u8;13] = [0x32, 0xff, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x49, 0xca, 0x00, 0x00, 0x01];
    let decoded = Gpdu { header: Gtpv1Header {msgtype:GPDU, length:5, teid:0, sequence_number:Some(18890), npdu_number: None, extension_headers:None}, tpdu: vec!(1) };
...
fn test_gpdu_missing_tpdu_unmarshal () {
    let encoded:[u8;12] = [0x32, 0xff, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x49, 0xca, 0x00, 0x00];
    assert_eq!(Gpdu::unmarshal(&encoded),Err(GTPV1Error::MessageLengthError));
}
