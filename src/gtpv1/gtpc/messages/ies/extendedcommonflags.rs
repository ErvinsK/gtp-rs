...
impl Default for ExtendedCommonFlags {
    fn default() -> Self {
        ExtendedCommonFlags { t: EXTCOMMONFLAGS, length:EXTCOMMONFLAGS_LENGTH, unauthenticated_imsi:false, ccrsi:false, cpsr: false, retloc: false, vb: false, pcri: false, bdwi: false, uasi: false}
    }
}
...
impl IEs for ExtendedCommonFlags {
    fn marshal (&self, buffer: &mut Vec<u8>) {
        ...
        let flags = self.clone().into_array().iter().map( |x| if *x {1} else {0}).enumerate().map( |(i,x)| x<<(7-i)).collect::<Vec<_>>().iter().sum::<u8>();
...
fn extcommonflags_ie_marshal_test () {
    ...
    let ie_to_marshal = ExtendedCommonFlags { t:EXTCOMMONFLAGS, length: EXTCOMMONFLAGS_LENGTH, uasi:false, bdwi:true, pcri:true, vb:false, retloc:false, cpsr:false, ccrsi:false, unauthenticated_imsi:true };
...
fn extcommonflags_ie_unmarshal_test () {
    ...
    let ie_unmarshalled = ExtendedCommonFlags { t:EXTCOMMONFLAGS, length: EXTCOMMONFLAGS_LENGTH, uasi:false, bdwi:true, pcri:true, vb:false, retloc:false, cpsr:false, ccrsi:false, unauthenticated_imsi:true };
    assert_eq!(ExtendedCommonFlags::unmarshal(&ie_to_unmarshal).unwrap(), ie_unmarshalled);
}