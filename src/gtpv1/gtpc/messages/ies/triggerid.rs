...
impl IEs for TriggerId {
    fn marshal(&self, buffer: &mut Vec<u8>) {
        ...
        buffer_ie.append(&mut self.triggerid.clone());
...
fn triggerid_ie_marshal_test() {
    ...
    let ie_to_marshal = TriggerId {
        t: TRIGGERID,
        length: 2,
        triggerid: vec![0x80, 0x80],
    };
...
fn triggerid_ie_unmarshal_test() {
    ...
    let ie_unmarshalled = TriggerId {
        t: TRIGGERID,
        length: 2,
        triggerid: vec![0x80, 0x80],
    };
    assert_eq!(
        TriggerId::unmarshal(&ie_to_unmarshal).unwrap(),
        ie_unmarshalled
    );
}
