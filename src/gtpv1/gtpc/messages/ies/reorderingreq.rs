...
impl Default for ReorderingRequired {
    fn default() -> Self {
        ReorderingRequired { t: REORDERING_REQUIRED, req: false }
    }
}
...
fn reordering_required_ie_marshal_test() {
    let ie_to_marshal=ReorderingRequired{ t: REORDERING_REQUIRED, req: true};
...
fn reordering_required_ie_unmarshal_test() {
    ...
    let ie_unmarshalled = ReorderingRequired { t: REORDERING_REQUIRED, req: true};
    assert_eq!(ReorderingRequired::unmarshal(&ie_to_unmarshal).unwrap(), ie_unmarshalled);
}