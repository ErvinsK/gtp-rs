...
impl Default for GGSNBackOffTime {
    fn default() -> Self {
        GGSNBackOffTime { t: GGSN_BACKOFF, length:GGSN_BACKOFF_LENGTH, timer_unit:0, timer_value:0}
    }
}
...
fn ggsn_backoff_ie_marshal_test () {
    ...
    let ie_to_marshal = GGSNBackOffTime { t:GGSN_BACKOFF, length: GGSN_BACKOFF_LENGTH, timer_unit:3, timer_value:31 };
...
fn ggsn_backoff_ie_unmarshal_test () {
    ...
    let ie_unmarshalled = GGSNBackOffTime { t:GGSN_BACKOFF, length: GGSN_BACKOFF_LENGTH, timer_unit:3, timer_value:31 };
    assert_eq!(GGSNBackOffTime::unmarshal(&ie_to_unmarshal).unwrap(), ie_unmarshalled);
}