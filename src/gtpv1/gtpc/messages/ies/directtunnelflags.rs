...
impl Default for DirectTunnelFlags {
    fn default() -> Self {
        DirectTunnelFlags { t: DTF, length:DTF_LENGTH, dti:false, gcsi:false, ei:false}
    }
}
...
fn dtf_ie_marshal_test () {
    ...
    let ie_to_marshal = DirectTunnelFlags { t:DTF, length: DTF_LENGTH, dti:true, gcsi:true, ei:false };
...
fn dtf_ie_unmarshal_test () {
    ...
    let ie_unmarshalled = DirectTunnelFlags { t:DTF, length: DTF_LENGTH, dti:true, gcsi:true, ei:false };
    assert_eq!(DirectTunnelFlags::unmarshal(&ie_to_unmarshal).unwrap(), ie_unmarshalled);
}