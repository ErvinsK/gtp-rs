...
impl Default for BearerControlMode {
    fn default() -> BearerControlMode {
        BearerControlMode { t: BEARER_CONTROL_MODE, length:BEARER_CONTROL_MODE_LENGTH, bearer_ctrl_mode:0 }        
    }
}
...
impl IEs for BearerControlMode {
    ...
    fn unmarshal (buffer:&[u8]) -> Result<Self, GTPV1Error> {
        if buffer.len()>=(BEARER_CONTROL_MODE_LENGTH+3) as usize {
            let mut data = BearerControlMode::default();
...
fn bearer_ctrl_mode_ie_unmarshal_test () {
    ...
    let test_struct = BearerControlMode { t:BEARER_CONTROL_MODE, length: BEARER_CONTROL_MODE_LENGTH, bearer_ctrl_mode: 0 };
