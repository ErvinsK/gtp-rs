...
impl Default for ChargingID {
    fn default() -> Self {
        ChargingID { t: CHARGING_ID, value: 0 }
    }
}
...
fn charging_id_ie_marshal_test() {
    let ie_to_marshal=ChargingID{ t: CHARGING_ID, value:0xff};
...
fn charging_id_ie_unmarshal_test() {
    ...
    let ie_unmarshalled = ChargingID { t: CHARGING_ID, value:0xff};
    assert_eq!(ChargingID::unmarshal(&ie_to_unmarshal).unwrap(), ie_unmarshalled);
}