            ...
            loop {
                if buffer[cursor]>=increment {    
                    match buffer[cursor] {
                                ...
                                TEARDOWN_IND => {
                                    match TeardownInd::unmarshal(&buffer[cursor..]) {
                                        Ok(i) => {
                                            if !msg_hash.contains_key(&buffer[cursor]) {
                                                increment = buffer[cursor];
                                                msg_hash.insert(buffer[cursor], 1);
...
fn delete_pdp_ctx_req_unmarshal_test() {
    let encoded:[u8;16]= [0x32, 0x14, 0x00, 0x08, 0x85, 0x92, 0xc6, 0x4a, 0x07, 0xe7, 0x00, 0x00, 0x13, 0xff, 0x14, 0x06];
    let decoded = DeletePDPContextRequest {
        header: Gtpv1Header { msgtype: DELETE_PDP_CONTEXT_REQUEST, length: 8, teid: 2240988746, sequence_number: Some(2023), npdu_number: None, extension_headers: None }, 
        cause: None,
        teardown: Some(TeardownInd { t: TEARDOWN_IND, teardown: true}),
        ...
        uli_timestamp: None, 
        private_extension: None };
    assert_eq!(DeletePDPContextRequest::unmarshal(&encoded).unwrap(),decoded);
}
...
fn delete_pdp_ctx_req_wrong_ie_order_unmarshal_test() {
    let encoded:[u8;16]= [0x32, 0x14, 0x00, 0x08, 0x85, 0x92, 0xc6, 0x4a, 0x07, 0xe7, 0x00, 0x00, 0x14, 0x06, 0x13, 0xff];
    assert_eq!(DeletePDPContextRequest::unmarshal(&encoded),Err(GTPV1Error::MessageInvalidMessageFormat));
}

#[test]
fn delete_pdp_ctx_req_missing_mandatory_ie_unmarshal_test() {
    let encoded:[u8;14]= [0x32, 0x14, 0x00, 0x06, 0x85, 0x92, 0xc6, 0x4a, 0x07, 0xe7, 0x00, 0x00, 0x13, 0xff];
    assert_eq!(DeletePDPContextRequest::unmarshal(&encoded),Err(GTPV1Error::MessageMandatoryIEMissing));
}