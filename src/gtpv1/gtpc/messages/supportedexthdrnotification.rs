...
fn test_supported_ext_hdr_notification_unmarshal () {
    let encoded:[u8;19] = [0x32, 0x1f, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x4b, 0x00, 0x00, 0x8d, 0x01, 0x00, 0x01, 0x02, 0x03, 0x04];
    ...
    assert_eq!(SupportedExtensionHeadersNotification::unmarshal(&encoded).unwrap(),decoded);
}

#[test]
fn test_supported_ext_hdr_notification_marshal () {
    let encoded:[u8;19] = [0x32, 0x1f, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x4b, 0x00, 0x00, 0x8d, 0x05, 0x00, 0x01, 0x02, 0x03, 0x04];
    let decoded:SupportedExtensionHeadersNotification = SupportedExtensionHeadersNotification { 
        header: Gtpv1Header {
            msgtype:SUPPORTED_EXTENSION_HEADERS_NOTIFICATION,
            length:11,
...
fn test_supported_ext_hdr_notification_resp_without_mandatory_ie_unmarshal () {
    let encoded:[u8;12] = [0x32, 0x1f, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x4b, 0x00, 0x00];
...
fn test_supported_ext_hdr_notification_with_incorrect_mandatory_ie_unmarshal () {
    let encoded:[u8;17] = [0x32, 0x1f, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x4b, 0x00, 0x00, 0x8d, 0x05, 0x00, 0x01, 0x02];
    assert_eq!(SupportedExtensionHeadersNotification::unmarshal(&encoded).unwrap_err(), GTPV1Error::MessageMandatoryIEMissing);
}
