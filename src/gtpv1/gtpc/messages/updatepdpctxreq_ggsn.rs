...
impl Messages for UpdatePDPContextRequestGGSN {
    ...
    fn marshal (self, buffer: &mut Vec<u8>) {
        ...
        if let Some(i) = self.end_user_address { i.marshal(buffer)}; 
        ...
        if let Some(i) = self.apn_restriction { i.marshal(buffer)}; 
        
        // Marshal MS Info Change Reporting IE

        if let Some(i) = self.ms_info_change { i.marshal(buffer)}; 
        ...
        if let Some(i) = self.bearer_ctrl_mode { i.marshal(buffer)}; 
...
fn update_pdp_ctx_req_ggsn_unmarshal_test() {
    ...
    let encoded:[u8;78]= [
        0x32,0x12,0x00,0x46,0x10,0x2b,0xdf,0x23,
        0xc0,0x86,0x00,0x00,0x02,0x09,0x41,0x50,
        0x01,0x72,0x67,0x35,0xf9,0x0e,0xbf,0x14,
        0x05,0x80,0x00,0x16,0xf1,0x8d,0x64,0x75,
        0x82,0x35,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,
        0xff,0xff,0x87,0x00,0x11,0x03,0x23,0x73,
        0x1f,0x93,0x96,0x86,0x86,0x74,0x83,0xff,
        0xff,0x00,0x00,0x00,0x00,0x00,0x94,0x00,
        0x01,0x40,0xbf,0x00,0x01,0x64
    ];
    let decoded = UpdatePDPContextRequestGGSN {
        header: Gtpv1Header { msgtype: 18, length: 70, teid: 271310627, sequence_number: Some(49286), npdu_number: None, extension_headers: None }, 
        imsi: Some(Imsi { t: 2, imsi: "901405102776539".to_string() }),
        recovery: Some(Recovery { t: 14, value: 191 }), 
        nsapi: Nsapi { t: 20, value: 5 }, 
        end_user_address: Some(EndUserAddress { t:END_USER_ADDRESS, length:22, pdp_type_org: IETF, pdp_type_nbr: IPV46, ipv4: Some(Ipv4Addr::new(100,117,130,53)), ipv6: Some(Ipv6Addr::new(0,0,0,0,0,0,0xffff,0xffff))}),
        pco: None, 
        qos: Some(Qos { t: 135, length: 17, arp: 3, qos: vec!(35, 115, 31, 147, 150, 134, 134, 116, 131, 255, 255, 0, 0, 0, 0, 0) }), 
        tft: None, 
        ...
        apn_ambr: None, 
        private_extension: None };
    assert_eq!(UpdatePDPContextRequestGGSN::unmarshal(&encoded).unwrap(),decoded);
}
...
fn update_pdp_ctx_req_ggsn_wrong_ie_order_unmarshal_test() {
    let encoded:[u8;78]= [
        0x32,0x12,0x00,0x46,0x10,0x2b,0xdf,0x23,
        0xc0,0x86,0x00,0x00,0x02,0x09,0x41,0x50,
        0x01,0x72,0x67,0x35,0xf9,
        0x14,0x05,
        0x0e,0xbf,
        0x80,0x00,0x16,0xf1,0x8d,0x64,0x75,
        0x82,0x35,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,
        0xff,0xff,0x87,0x00,0x11,0x03,0x23,0x73,
        0x1f,0x93,0x96,0x86,0x86,0x74,0x83,0xff,
    ...
    assert_eq!(UpdatePDPContextRequestGGSN::unmarshal(&encoded),Err(GTPV1Error::MessageInvalidMessageFormat));
}

#[test]
fn update_pdp_ctx_req_ggsn_missing_mandatory_ie_unmarshal_test() {
    let encoded:[u8;76]= [
        0x32,0x12,0x00,0x44,0x10,0x2b,0xdf,0x23,
        0xc0,0x86,0x00,0x00,0x02,0x09,0x41,0x50,
        0x01,0x72,0x67,0x35,0xf9,0x0e,0xbf,
        0x80,0x00,0x16,0xf1,0x8d,0x64,0x75,
        0x82,0x35,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,
        0xff,0xff,0x87,0x00,0x11,0x03,0x23,0x73,
    ...
    assert_eq!(UpdatePDPContextRequestGGSN::unmarshal(&encoded),Err(GTPV1Error::MessageMandatoryIEMissing));
}
