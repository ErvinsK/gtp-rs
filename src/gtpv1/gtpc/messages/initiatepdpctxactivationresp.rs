...
fn init_pdp_ctx_activ_resp_unmarshal_test() {
    let encoded:[u8;14]= [
        0x32,0x17,0x00,0x06,0x37,0x38,0xbf,0x7a,
        0x9b,0xcf,0x00,0x00,0x01,0x80
    ];
    let decoded = InitiatePDPContextActivationResponse { 
        header: Gtpv1Header { msgtype: INITIATE_PDP_CTX_ACTIVATION_RESPONSE, length: 6, teid: 926465914, sequence_number: Some(39887), npdu_number: None, extension_headers: None }, 
        cause: Cause { t: 1, value: 128 },
        pco: None,
        private_extension: None };
    assert_eq!(InitiatePDPContextActivationResponse::unmarshal(&encoded).unwrap(),decoded);
}
...
fn init_pdp_ctx_activ_resp_wrong_ie_order_unmarshal_test() {
    let encoded:[u8;16]= [
        0x32,0x17,0x0,0x08,0x37,0x38,0xbf,0x7a,
        0x9b,0xcf,0x0,0x0,0x08, 0xfe, 0x01,0x80
    ];
    assert_eq!(InitiatePDPContextActivationResponse::unmarshal(&encoded),Err(GTPV1Error::MessageInvalidMessageFormat));
}

#[test]
fn init_pdp_ctx_activ_resp_missing_mandatory_ie_unmarshal_test() {
    let encoded:[u8;12]= [
        0x32,0x17,0x00,0x04,0x37,0x38,0xbf,0x7a,
        0x9b,0xcf,0x00,0x00
    ];
    assert_eq!(InitiatePDPContextActivationResponse::unmarshal(&encoded),Err(GTPV1Error::MessageMandatoryIEMissing));
}