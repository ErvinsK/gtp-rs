...
fn delete_pdp_ctx_resp_unmarshal_test() {
    let encoded:[u8;14]= [0x32, 0x15, 0x00, 0x06, 0x0c, 0xbf, 0x0a, 0x50, 0x07, 0xe7, 0x00, 0x00, 0x01, 0x80];
    let decoded = DeletePDPContextResponse {
        header: Gtpv1Header { msgtype: DELETE_PDP_CONTEXT_RESPONSE, length: 6, teid: 213846608, sequence_number: Some(2023), npdu_number: None, extension_headers: None }, 
        cause: Cause { t: CAUSE, value: 128 },
        pco: None, 
        uli: None, 
        ms_timezone: None, 
        uli_timestamp: None, 
        private_extension: None };
    assert_eq!(DeletePDPContextResponse::unmarshal(&encoded).unwrap(),decoded);
}
...
fn delete_pdp_ctx_resp_wrong_ie_order_unmarshal_test() {
    let encoded:[u8;21]= [0x32, 0x15, 0x00, 0x0d, 0x0c, 0xbf, 0x0a, 0x50, 0x07, 0xe7, 0x00, 0x00, 0xd6, 0x00, 0x04, 0xee, 0x6b, 0x28, 0x00, 0x01, 0x80];
    assert_eq!(DeletePDPContextResponse::unmarshal(&encoded),Err(GTPV1Error::MessageInvalidMessageFormat));
}

#[test]
fn delete_pdp_ctx_resp_missing_mandatory_ie_unmarshal_test() {
    let encoded:[u8;19]= [0x32, 0x15, 0x00, 0x0b, 0x0c, 0xbf, 0x0a, 0x50, 0x07, 0xe7, 0x00, 0x00, 0xd6, 0x00, 0x04, 0xee, 0x6b, 0x28, 0x00];
    assert_eq!(DeletePDPContextResponse::unmarshal(&encoded),Err(GTPV1Error::MessageMandatoryIEMissing));
}