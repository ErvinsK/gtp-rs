...
fn pdu_notification_reject_resp_wrong_ie_order_unmarshal_test() {
    ...
    assert_eq!(PDUNotificationRejectResponse::unmarshal(&encoded),Err(GTPV1Error::MessageInvalidMessageFormat));
}
...
fn pdu_notification_reject_resp_missing_mandatory_ie_unmarshal_test() {
    ...
    assert_eq!(PDUNotificationRejectResponse::unmarshal(&encoded),Err(GTPV1Error::MessageMandatoryIEMissing));
}